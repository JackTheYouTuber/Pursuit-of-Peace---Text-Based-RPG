"""
engine.py — the core orchestrator. The general who gives orders.

Does not validate. Does not implement game rules.
Asks the Router what to call, tells the Managers to execute it,
stores results in State, asks ViewBuilder for the UI state dict.
"""
import random
...
class Engine:
    def __init__(self, data_registry, profile_name: str = None, logger=None):
        ...
        # Complex managers
        from app.logic.complex.buff_mgr    import BuffMgr
        from app.logic.complex.item_mgr    import ItemMgr
        ...
        self._buffs   = BuffMgr()
        self._items   = ItemMgr(data_registry.items, self._buffs, data_registry.config)
        self._player  = PlayerMgr(data_registry.items, data_registry.lore,
                                  self._buffs, data_registry.config)
        self._combat  = CombatMgr(data_registry.items, self._buffs)
        self._dungeon = DungeonMgr(data_registry.dungeon)

        # View builder (logic-side, produces UI state dicts)
        from app.logic.core.view_builder import ViewBuilder
        self._view = ViewBuilder(data_registry, self._buffs)
    ...
    def do_city_action(self, action_id: str) -> Tuple[bool, str]:
        ...
        if is_nav(action_id):
            ps = set_location(ps, nav_target(action_id))
            self._state.set_player(ps)
            return True, ""
        ...
        if changed:
            rollover = self._clock.tick()
            if rollover:
                new_ps, rm = self._player.year_rollover(self._state.player())
                if new_ps is None:
                    return True, rm   # exile — caller handles GAME OVER
    ...
    def _dispatch_city(self, action_id: str, ps: Dict) -> Tuple[bool, str]:
        if action_id == "rest":
            new_ps, msg = self._player.rest(ps)
        elif action_id == "hear_rumors":
            new_ps, msg = self._player.hear_rumors(ps)
        elif action_id == "take_bath":
            new_ps, msg = self._player.take_bath(ps)
        elif action_id == "go_fishing":
            new_ps, msg = self._player.fish(ps)
        elif action_id == "pay_taxes":
            new_ps, msg = self._player.pay_taxes(ps)
            ...
            new_ps = set_location(ps, "dungeon_entrance")
        elif action_id == "repair_weapon":
            new_ps, msg = self._items.repair(ps, "equipped_weapon")
        elif action_id == "repair_armor":
            new_ps, msg = self._items.repair(ps, "equipped_armor")
        else:
            return False, f"Action '{action_id}' not handled."

        self._state.set_player(new_ps)
        return True, msg if action_id != "enter_dungeon" else "You descend into the darkness."

    def _dispatch_prefixed(self, prefix: str, param: str, ps: Dict) -> Tuple[bool, str]:
        if prefix == "use_item":
            new_ps, msg = self._items.use(ps, param)
        elif prefix == "equip_item":
            new_ps, msg = self._items.equip(ps, param)
        elif prefix == "unequip":
            new_ps, msg = self._items.unequip(ps, param)
        elif prefix == "sell_item":
            new_ps, msg = self._items.sell(ps, param)
        elif prefix == "buy_item":
            new_ps, msg = self._items.buy(ps, param)
        else:
            return False, f"Unknown prefixed action: {prefix}"
        self._state.set_player(new_ps)
        return True, msg
    ...
    def do_dungeon_action(self, action_id: str) -> Tuple[bool, str, Optional[Dict]]:
        ...
        if action_id in ("flee_dungeon", "dungeon_exit"):
            ...
            ps = set_location(self._state.player(), "city_entrance")
            self._state.set_player(ps)
        ...
        if action_id == "enter_combat":
            ...
            if room and room.get("enemy"):
                cs = self._combat.start(
                    room["enemy"], ps.get("hp", 20), ps.get("max_hp", 20),
                    ps.get("equipped_weapon"), ps.get("equipped_armor"))
        ...
        if action_id == "next_room":
            ...
            if at_exit:
                ps, _ = expire_run_buffs(self._state.player())
                self._state.set_player(ps)
                self._state.clear_dungeon()
                ps = set_location(self._state.player(), "city_entrance")
                self._state.set_player(ps)
        ...
        if action_id.startswith("use_item:"):
            ...
            new_ps, msg = self._items.use(ps, item_id)
            self._state.set_player(new_ps)
            return True, msg, None
    ...
    def do_combat_action(self, action_id: str) -> Tuple[bool, str, bool, bool]:
        ...
        if action_id == "attack":
            new_ps, snapshot, ended, dead = self._combat.attack(ps)
            self._state.set_player(new_ps)
        ...
        if action_id.startswith("use_item:"):
            ...
            if cs:
                cs["player_current_hp"] = new_ps.get("hp", 20)
    ...
    def _on_player_death(self):
        self._combat.clear()
        self._state.clear_dungeon()
        if self._reg.profiles.exists(self._name or ""):
            self._reg.profiles.delete(self._name)
            if self._logger: self._logger.info(f"Profile '{self._name}' deleted (death).")
        self._name = None
        self._clock.reset()
    ...
    def get_view_state(self, view_name: str) -> Dict:
        if view_name == "city":
            return self._view.build_city(self._state.player(),
                                         self._reg.location, self._reg.lore)
    ...
    def get_player_panel_data(self) -> Dict:
        return self._view.build_player_panel(self._state.player(), self._buffs)
