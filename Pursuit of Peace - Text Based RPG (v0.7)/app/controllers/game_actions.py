...
class GameActions:
    ...
    v0.7 — Handles player_dead flag from do_combat_action ([ISSUE-002]).
           Shows game-over dialog and resets to city view on death or exile.
    """
    ...
    def on_location_action(self, action_id: str):
        ...
        if msg:
            # Check for exile keyword — treat as game over
    ...
    def on_combat_action(self, action_id: str):
        ...
        if player_dead:
            self._show_game_over(
                ...
                "Your body is dragged from the dungeon by city guards.\n\n"
                "GAME OVER"
            )
            return
    ...
    def _show_game_over(self, msg: str):
        """Show a game-over dialog, then reset to city view with fresh state."""
        messagebox.showwarning("Game Over", msg)
        # Reset player state via engine profile restart
